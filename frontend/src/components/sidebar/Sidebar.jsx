        ...
        <div className="relative">
          ...
          {showMenu && (
            <div
              ...
              style={{ background: "var(--bg-card)", border: "1px solid var(--bg-elevated)", minWidth: 160 }}
              onMouseLeave={() => setShowMenu(false)}
            >
              <div className="px-4 py-2 border-b" style={{ borderColor: "var(--bg-elevated)" }}>
...
export function Avatar({ name = "?", size = 10, online = false }) {
  ...
  const [a, b] = colors[(name.charCodeAt(0) || 65) % colors.length];
  const px = size * 4;
  return (
    <div className="relative flex-shrink-0">
      ...
      {online && (
        <span className="absolute bottom-0 right-0 w-3 h-3 rounded-full border-2"
          style={{ background: "var(--online)", borderColor: "var(--bg-sidebar)", boxShadow: "0 0 0 1px var(--online)" }} />
      )}
    </div>
  );
}
